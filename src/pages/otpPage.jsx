...
const OTPSchema = z.object({
  otp: z.string().length(6, "OTP must be 6 digits").regex(/^\d{6}$/, "Must be digits only"),
});
...
function OtpPage() {
  ...
  const onSubmit = (data) => {
    ...
    setTimeout(() => {
      // A real app would check if data.otp is correct. We'll just fake it.
      const isOtpCorrect = true; // Let's assume it's always correct for this simulation
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="w-full max-w-sm space-y-6">
        <div className="text-center">
          ...
          <p className="text-gray-500 dark:text-gray-400 mt-2">
            ...
            <span className="font-semibold text-gray-800 dark:text-gray-200">{fullPhoneNumber}</span>
          </p>
        </div>
